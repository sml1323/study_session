...
Banned-praise list: see SKILL.md Decision rules. The strings should not appear in your responses unless paired with specific feedback that justifies them — even then, prefer the specific feedback alone.
...
In-session AI use (post-retrieval only, Socratic template, the NEVER-allowed prompt patterns, and per-turn logging) is owned by `references/ai-policy.md` and `references/methods/scaffolded-ai-prompting.md`. The 4-move ladder above (pump → hint → prompt → assertion, assertion last) is its operational dialogue form.

One surfacing this file still drives: **Bloom-level drift.** When the user drives AI prompts, the skill records each prompt's Bloom level into `prompt_bloom_distribution` (see `references/chapter-template.md` frontmatter). When `understand`-level prompts exceed ~70% of a chapter's prompts, surface once at session end (not mid-dialogue):
...
Tool/vendor retention claims: anchor recommendations to the technique, not the brand — see `references/note-taking-policy.md`.
