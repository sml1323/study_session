...
# System-emitted from the deterministic hook log at compose time (append-only, deduped).
# Generated by `scripts/analyze_references.py --emit-frontmatter --chapter <this-file>`.
# Do not author manually — the hook log (~/study-journal/.session-log/<KST-date>.jsonl) is the source of truth.
references_touched:
  - pdp-loop.md§TUTOR
